...
def feature_extraction(raw_data_features, raw_data_labels, timestamps):
  ...
  for i in range(0, accel_magnitudes.shape[0]-segment_size, 64):
    ...
    segment_fft_powers = np.abs(np.fft.fft(segment))**2 #TODO check the double-sided spectrum
    segment_fftfreq    = np.fft.fftfreq(len(segment),1/32)
    peakfreq           = segment_fftfreq[np.argmax(segment_fft_powers)]
...
def five_fold_cross_validation(features, labels):
  ...
  confusion_matrix       = np.zeros((len(np.unique(true_labels)),
  ...
  confusion_matrix_full = np.copy(confusion_matrix)
  ...
  return confusion_matrix_full, true_labels, predicted_labels

def evaluate_generalized_model(X_train, Y_train, X_test, Y_test):
  clf = DecisionTreeClassifier(random_state=42).fit(X_train, Y_train) 
  #GradientBoostingClassifier(n_estimators=51,random_state=42).fit(X_train, Y_train)
  #RandomForestClassifier(n_estimators=51,random_state=42).fit(X_train, Y_train) 
  ...
  confusion_matrix = np.zeros((len(np.unique(Y_test)), len(np.unique(Y_pred))))
  ...
  return confusion_matrix_full, Y_test, Y_pred
  
def precision_recall_from_confumat(confu_mat):
  #compute precision, recall, and accuracy from the confusion matrix
  all_activity_results = []
  for each_activity in activity_indices:
      activity_idx = activity_indices[each_activity]
      activity_tp  = confu_mat[activity_idx,activity_idx]
      activity_fp  = np.sum(confu_mat[:,activity_idx])-activity_tp
      activity_fn  = np.sum(confu_mat[activity_idx,:])-activity_tp
      
      activity_precision = np.round(activity_tp/(activity_tp + activity_fp),2)
      activity_recall    = np.round(activity_tp/(activity_tp + activity_fn),2)
      all_activity_results.append({'activity':each_activity,'precision':activity_precision,
                                   'recall':activity_recall})
  all_activity_results_pd = pd.DataFrame(all_activity_results)
  return all_activity_results_pd
...
if __name__ == "__main__":
  ...
  confu_mat_personalmodel, true_labels_personalmodel,\
     predicted_labels_personalmodel  = five_fold_cross_validation(features, labels)
  #compute classification performance like precision, recall, accuracy for comparison
  target_names = pd.Series(activity_indices).to_frame().sort_values(by=0).index.values
  personalmodel_classification_report= classification_report(true_labels_personalmodel,
                                                             predicted_labels_personalmodel,
                                                             target_names=target_names)
  all_activity_results_personal_pd = precision_recall_from_confumat(confu_mat_personalmodel)
  personalmodel_accuracy  = np.trace(confu_mat_personalmodel)/np.sum(confu_mat_personalmodel)
  ...
  confu_mat_generalizedmodel, true_labels_generalizedmodel,\
      predicted_labels_generalizedmodel = evaluate_generalized_model(X_train, Y_train, X_test, Y_test)
  generalizedmodel_classification_report= classification_report(true_labels_generalizedmodel,
                                                             predicted_labels_generalizedmodel,
                                                             target_names=target_names)
  all_activity_results_generalized_pd = precision_recall_from_confumat(confu_mat_generalizedmodel)
  generalizedmodel_accuracy = np.trace(confu_mat_generalizedmodel)/np.sum(confu_mat_generalizedmodel)
